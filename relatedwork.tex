...
\begin{table}[t]
	...
	\begin{tabular}{p{2cm}|p{6cm}|p{1cm}|p{1cm}|p{2cm}}
		\toprule
		主要思想\centering & 测试覆盖指标名称\centering & 复杂度 & 可解释性 & 文献 \\
		\midrule
		单个神经元激活值\centering & 神经元覆盖率，K区间覆盖率，重要性驱动覆盖\centering & $O(n)$ & 无 & DeepXplore~\citess{Pei2019DeepXplore}，DeepGauge~\citess{ma2018deepgauge}，IDC~\citess{Gerasimou2020Importance} \\
		神经元激活值边界\centering & 神经元边界覆盖率，强神经元覆盖率\centering & $O(n)$ & 无 & DeepGauge~\citess{ma2018deepgauge} \\
		神经元激活值分布\centering & 重要神经元覆盖，意外覆盖，神经元激活向量距离，平均偏差\centering & $O(nm)$ & 无 & DeepGauge~\citess{ma2018deepgauge}，SADL~\citess{Kim2019Guiding}，DeepInspect~\citess{Tian2019Testing} \\
		神经元激活通路\centering & 重要神经元通路覆盖率，强激活通路覆盖率，符号-符号覆盖，距离-符号覆盖，符号-值覆盖，距离-值覆盖\centering & $O(nl)$ & 无 & DeepGauge~\citess{ma2018deepgauge}，DeepPath~\citess{Wang2019DeepPath}，DeepCover~\citess{Sun2018Testing} \\
		神经元状态\centering & 状态级别覆盖，转换级别覆盖\centering & $O(n)$ & 无 & DeepCruiser~\citess{Du2018DeepCruiser} \\
		神经元组合分布\centering & $t-way$组合稀疏覆盖，$t-way$组合密集覆盖，$(p,t)$完整性覆盖\centering & $O(n^2)$ & 无 & DeepCT~\citess{ma2019deepct} \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[t]
	...
	\begin{tabular}{c|p{5cm}|p{1cm}|c|p{3cm}}
		\toprule
		算法思想 & 测试目标\centering & 测试对象 & 是否可变异 & 文献 \\
		\midrule
		模糊测试 & 神经元边界覆盖，神经元覆盖率，L2距离，强神经元覆盖率，k区间覆盖率，重要神经元覆盖率\centering & 图像分类，自然语言识别 & 是 & TensorFuzz~\citess{Odena2019TensorFuzz}，DLFuzz~\citess{Guo2018DLFuzz}，DeepHunter~\citess{xie2019coverage} \\
		Concolic测试 & 神经元覆盖率，MC/DC覆盖\centering & 图像分类 & 是 & DeepConcolic~\citess{Sun2018Concolic} \\
		符号执行 & -\centering & 代码分类 & 是 & DeepCheck~\citess{Gopinath2018Symbolic} \\
		对抗样本 & 蒙特卡洛树搜索，分类置信度，分类准确率、失真度，神经元覆盖率\centering & 图像分类 & 是 & Xiao等人~\citess{Xiao2018Spatially}，Wicker等人~\citess{Wicker2018FeatureGuided}，OptMargin~\citess{He2018Decision} \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{基于变异测试。}Wang等人~\citess{Wang2021Prioritizing}，结合learning-to-rank方法（一种解决排序问题的监督机器学习方法）来智能地组合变异结果用于有效的测试输入优选，在此基础上提出了方法PRIMA，认为能够杀死许多变异测试模型并产生不同预测结果的测试输入数据更有可能揭示DNN模型的错误，应该优先考虑此类数据。
Ma等人~\citess{Ma2018DeepMutation}借鉴传统软件变异测试的思想,将其应用到针对DNN系统的测试中,提出了DeepMutation，可以对测试用例集质量进行有效的定量分析,帮助开发人员提高选择测试数据的能力。

\item \textbf{基于神经元状态统计。}Liu等人~\citess{Liu2022DeepState}提出一种针对RNN神经网络结构的测试集优选方法DeepState，基于RNN的神经元状态选择数据，通过捕获RNN中神经元的状态变化来识别可能错误分类的测试数据集，进一步地设计了一种测试选择方法，能够从大型数据集中获得具有强大故障检测和模型改进能力的测试数据集。
...
\item \textbf{基于边界聚类。}Shen等人~\citess{Shen2020MultipleBoundary}提出了多边界聚类优选方法MCP，用于将测试数据聚类到深度学习模型的多个边界区域中，指定优先级从所有边界区域中均匀地选择样本以确保每个边界重建都有足够的有用样本，以此来更加高效地实现深度学习模型的再训练过程。
\end{itemize}
...
\begin{table}[t]
	...
	\begin{tabular}{c|c|c}
		...
		算法思想 & 测试数据集 & 文献 \\
		\midrule
		基于变异测试 & x & PRIMA~\citess{Wang2021Prioritizing}，DeepMutation~\citess{Ma2018DeepMutation} \\
		基于神经元状态统计 & x & DeepState~\citess{Liu2022DeepState}，DeepGini~\citess{Feng2020DeepGini}，DAT~\citess{Hu2022AnEmpirical}，Byun等人~\citess{Byun2019Input}，ATS~\citess{Gao2022Adaptive} \\
		基于边界聚类 & x & MCP~\citess{Shen2020MultipleBoundary} \\
		\bottomrule
	\end{tabular}
\end{table}
